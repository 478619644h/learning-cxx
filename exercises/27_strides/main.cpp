...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    for (auto stride_it = strides.rbegin() + 1; stride_it != strides.rend(); ++stride_it, ++it) {
        *stride_it = (*it) * *(stride_it - 1);
        std::cout << *stride_it << std::endl;
    }
...
// 自定义输出函数
void print_vector(const std::vector<udim> &vec) {
    std::cout << "[";
